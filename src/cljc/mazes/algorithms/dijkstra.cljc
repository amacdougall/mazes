...
(spec/def ::current ::grid/cell)
(spec/def ::destination ::grid/cell)
(spec/def ::distance int?)
(spec/def ::unvisited (spec/coll-of ::grid/cell :kind set?))
(spec/def ::distances (spec/map-of ::grid/cell ::distance :conform-keys true))
(spec/def ::step-values (spec/keys :req [::grid/grid ::unvisited ::distances ::current]))
...
(defn get-initial-values
  ...
  algorithm."
  [grid origin]
  (let [all-cells (grid/all-cells grid)
...
(spec/fdef get-initial-values
  :args (spec/cat :grid ::grid/grid :origin ::grid/cell)
  :ret ::step-values)
...
(defn step
  ...
  [{:keys [::grid/grid ::unvisited ::distances ::current] :as step-values}]
  (let [distance-updates ; a seq of [k v] pairs
        (map (fn [cell]
             ...
             (grid/linked-cells grid current))
...
(defn solve
  ...
  map of [::grid/cell int?] map-entries; e.g. {<cell> <int>, ...}."
...
(defn path
  "Given a grid and start and end cells, uses Dijkstra's algorithm to produce a
  collection of ::grid/direction items showing the most efficient route from the
